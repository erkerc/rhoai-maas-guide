...
oc apply -f manifests/03-rhoai-config/dscinitialization.yaml
oc apply -f manifests/03-rhoai-config/datasciencecluster.yaml
----
...
oc apply -f manifests/03-rhoai-config/odh-dashboard-config.yaml
----

On subsequent runs (CRDs already exist), `oc apply -k manifests/03-rhoai-config/` works in one shot.
...
NOTE: The `maas-api` deployment does not exist yet at this stage. It will be created automatically once the PostgreSQL database and `maas-db-config` secret are configured in xref:04-maas-platform.adoc[Phase 4]. You can verify it after completing Phase 4.
