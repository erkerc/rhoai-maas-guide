...
[NOTE]
====
*Non-cloud clusters (baremetal/OpenStack/RHPDS/SNO):* If the Gateway stays in `AddressNotAssigned` because no cloud load-balancer provisions an external IP, you need *MetalLB* to assign a LoadBalancer IP. See xref:01-prerequisites.adoc[Phase 1: MetalLB for Non-Cloud Clusters].
